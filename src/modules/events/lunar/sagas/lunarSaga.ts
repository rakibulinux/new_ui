...
import { awardData, awardError, AwardFetch, lotData, lotError, LotFetch, lunarError, rewardData, RewardPost } from '../actions';
...
export function* rewardPostSaga(action: RewardPost) {
	try {
		if (!action.payload.uid || !action.payload.txid) {
			//console.log('Uid or txid empty');
		} else {
