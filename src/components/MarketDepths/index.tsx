...
const CustomTooltip = (props: CustomToolTipProps) => {
	...
	if (active) {
		...
		const currData = payloadData ? external.find((entry: KeyValuePairMarketDepths) => entry.name === payloadData.name) : null;
				...
				<ResponsiveContainer width="100%" height={settings.height}>
					<AreaChart data={data} margin={{ top: 20, right: 30, left: 0, bottom: 20 }}>
						...
						{hideCartesianGrid ? null : (
							<CartesianGrid stroke={colorSettings.strokeGridColor} strokeDasharray="1 1" fill="url(#fillGrid)" />
						)}
						<XAxis
