...
describe('History saga', () => {
	...
	const url = `/account/history?currency=${fakeRequest.currency}&filter=${encodeURIComponent(fakeRequest.filter)}&time_from=${
		fakeRequest.time_from
	}&time_to=${fakeRequest.time_to}`;
	const mocksHistory = () => {
		mockAxios.onGet(url).reply(200, fakeSuccessPayload.list);
	};

	const expectedActionsFetch = [fetchNewHistory(fakeRequest), successNewHistory(fakeSuccessPayload)];
	const expectedActionsError = [fetchNewHistory(fakeRequest), failNewHistory(error), alertPush(fakeError)];
