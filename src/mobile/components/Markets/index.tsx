...
const MarketsComponent : React.FC<MarketsProps> = ({headers}) => {
    ...
    React.useEffect(() => {
        const marketsFilter = currentTabIndex === 0 ? markets : markets.filter(market => market.quote_unit === currentBidUnitsList[currentTabIndex]);
    ...
    const renderTabs = () => currentBidUnitsList.map((name,i) => ({
    ...
    const renderTab = () => {
        return (
            <Table headers={headers ? headers : defaultHeader} markets={currentMarketsFilter}/>
        );
    };
