...
export const HotAnnouncement: React.FC = () => {
	...
	const renderAnnouncementright = () => {
		return (
			<div className="article">
				...
				{announcements.data.slice(0, 10).map((announcement, index) => {
					return (
						<div className="hot-article-item" key={index}>
							<div className="hot-article-item__title">
								<Link className="hot-article-item__title-link" to={'/announcement/detail/' + announcement.id}>
									{announcement.title}
								</Link>
							</div>
						</div>
	...
	return <React.Fragment>{renderAnnouncementright()}</React.Fragment>;
};
