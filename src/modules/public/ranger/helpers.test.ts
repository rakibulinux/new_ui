...
describe('ranger helpers', () => {
	...
	describe('streamsBuilder', () => {
		...
		it('returns public streams with market', () => {
			expect(streamsBuilder(false, [], marketExample)).toEqual(['global.tickers', 'abcdefg.trades', 'abcdefg.update']);
		});

		it('includes private streams', () => {
			expect(streamsBuilder(true, [], undefined)).toEqual(['global.tickers', 'order', 'trade', 'deposit_address']);
		});
