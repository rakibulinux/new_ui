...
export const HomePageScreen = () => {
		...
		const drawMarketLines = async () => {
			try {
				...
				/*                 const klines4 = await fetchMarketsKlines(marketIds[3], from, to);
                                const klines5 = await fetchMarketsKlines(marketIds[4], from, to);
                                const klines6 = await fetchMarketsKlines(marketIds[5], from, to);
                                const klines7 = await fetchMarketsKlines(marketIds[6], from, to); */
	...
	const findIcon = (currencyId: string): string => {
		const currency = currencies.find(
			(currencyParam: Currency) => currencyParam.id.toLowerCase() === currencyId.toLowerCase(),
		);
		try {
	...
	const renderMarket = () => {
		const MarketChart = (data: any, marketIDParam: string) => {
			const market = markets.find(
				marketParam => marketParam.id.toLowerCase() === marketIDParam.split('/').join('').toLowerCase(),
			);
							...
							<div className="row">
								<div className="col-12 d-flex justify-content-between">
									<div>
										...
										<span
											style={{ fontSize: '1.2rem', fontWeight: 'bold', margin: '5px' }}
		...
		return (
			<Section>
				<div className="container" style={{ backgroundColor: '#313445ff', padding: '3rem 1.5rem', borderRadius: '1rem' }}>
					<div className="row">
						<div className="col-3">{MarketChart(KlineState1, marketIds[0])}</div>

						<div className="col-3">{MarketChart(KlineState2, marketIds[1])}</div>
						<div className="col-3">{MarketChart(KlineState3, marketIds[2])}</div>

						{/* <div className="col-3">
                            {MarketChart(KlineState4, marketIds[3])}
                        </div> */}
