...
describe('OrderForm', () => {
	...
	it('should call onSubmit callback', () => {
		...
		expect(
			onSubmit.calledOnceWith({ type: 'buy', orderType: 'Limit', price: nextState.price, amount: '1.5', available: 50 }),
		).toBeTruthy();
	});
	...
	const findInputPrice = (wrapper: ShallowWrapper) =>
		wrapper.find('.cr-order-item').at(1).find('div').first().children().first();
