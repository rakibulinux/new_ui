...
import { ClaimData, ClaimError, ClaimFetch, ClaimFetchUserId } from '../actions';
...
export function* getClaimUserIdSaga(action: ClaimFetchUserId) {
	try {
		const airdrop = yield axios.get<Claim>(
			`claim/getByUid/airdrop_id=${action.payload.airdrop_id}&user_uid=${action.payload.user_uid}`,
		);
