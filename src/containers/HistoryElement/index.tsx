...
import { localeDate, preciseData, setDepositStatusColor, setTradesType, setWithdrawStatusColor } from '../../helpers';
		...
		switch (type) {
			...
			case 'trades': {
				...
				return [
					...
					marketName,
					<Decimal key={id} fixed={marketToDisplay.price_precision}>
						{price}
					</Decimal>,
					<Decimal key={id} fixed={marketToDisplay.amount_precision}>
						{amount}
					</Decimal>,
					<Decimal key={id} fixed={marketToDisplay.amount_precision}>
						{total}
...
export const HistoryElement = compose(injectIntl, connect(mapStateToProps, mapDispatchToProps))(HistoryComponent) as any; // tslint:disable-line
