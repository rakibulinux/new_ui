...
export function* waitingAirdropSaga(action: WaitingAirdropFetch) {
	try {
		const airdrop = yield axios.get<Airdrop>(
			`airdrop/fetch/waiting/page=${action.payload.page - 1}&size=${action.payload.size}`,
		);
		yield put(airdropData(airdrop.data));
	} catch (error) {
		yield put(airdropError(error));
...
export function* openingAirdropSaga(action: OpeningAirdropFetch) {
	try {
		const airdrop = yield axios.get<Airdrop>(
			`airdrop/fetch/opening/page=${action.payload.page - 1}&size=${action.payload.size}`,
		);
		yield put(airdropData(airdrop.data));
	} catch (error) {
		yield put(airdropError(error));
...
export function* deliveringAirdropSaga(action: DeliveringAirdropFetch) {
	try {
		const airdrop = yield axios.get<Airdrop>(
			`airdrop/fetch/delivering/page=${action.payload.page - 1}&size=${action.payload.size}`,
		);
		yield put(airdropData(airdrop.data));
	} catch (error) {
		yield put(airdropError(error));
...
export function* deliveredAirdropSaga(action: DeliveredAirdropFetch) {
	try {
		const airdrop = yield axios.get<Airdrop>(
			`airdrop/fetch/delivered/page=${action.payload.page - 1}&size=${action.payload.size}`,
		);
		yield put(airdropData(airdrop.data));
	} catch (error) {
		yield put(airdropError(error));
