...
class WalletsComponent extends React.Component<Props, WalletsState> {
	...
	private handleWithdraw = () => {
		...
		const feeCurrency = eth_fee.find(cur => cur.currency_id === currency);
	...
	private renderDeposit = (isAccountActivated: boolean) => {
		...
		const textConfirmation = this.props.intl.formatMessage(
		...
		const textMinDeposit = `${this.translate(
			'page.body.wallets.tabs.deposit.ccy.message.mindeposit',
		)} ${mindeposit} ${currency.toUpperCase()}`;

		const textDepositFee = `${this.translate('page.body.wallets.tabs.deposit.ccy.message.depositfee')} ${Number(
		...
		const checkDepositFee =
			Number(currencyItem.deposit_fee) !== 0
				? textDepositFee
				: `${this.translate('page.body.wallets.tabs.deposit.ccy.message.depositfee')} 0`;
		...
		const buttonLabel = `
              ${this.translate('page.body.wallets.tabs.deposit.ccy.button.generate')} ${currency.toUpperCase()} ${this.translate(
	...
	private renderWithdrawContent = () => {
		...
		const eth = wallets.find(walletParam => walletParam.currency.toLowerCase() === 'eth');
		const ethBallance = eth ? eth.balance : undefined;
		const { currency, fee, type } = wallet;
		const fixed = (wallet || { fixed: 0 }).fixed;

		const feeCurrency = this.props.eth_fee.find(cur => cur.currency_id === currency);

		const ethFee = feeCurrency ? feeCurrency.fee : undefined;
