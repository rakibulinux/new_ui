...
describe('Open Orders reducer', () => {
	...
	it('should handle USER_OPEN_ORDERS_ERROR', () => {
		...
		expect(openOrdersReducer(initialState, actions.userOpenOrdersError())).toEqual(expectedState);
	});
	...
	describe('USER_OPEN_ORDERS_APPEND', () => {
		...
		it('inserts order', () => {
			...
			expect(openOrdersReducer(initialOpenOrdersState, actions.userOpenOrdersAppend(newOrder))).toEqual(expectedState);
		});
	});
	...
	describe('USER_OPEN_ORDERS_UPDATE', () => {
		...
		it('insert new order by ID', () => {
			...
			expect(openOrdersReducer(initialOpenOrdersState, actions.userOpenOrdersUpdate(newOrderEvent))).toEqual(expectedState);
		});

		it('insert new order by UUID', () => {
			const list = insertOrUpdate([], convertOrderEvent(newOrderWithUUIDEvent));
			const expectedState: OpenOrdersState = { ...initialOpenOrdersState, list };
			expect(openOrdersReducer(initialOpenOrdersState, actions.userOpenOrdersUpdate(newOrderWithUUIDEvent))).toEqual(
				expectedState,
			);
		});
		...
		it('does not insert done order', () => {
			const expectedState: OpenOrdersState = { ...initialOpenOrdersState, list: [] };
			expect(
				openOrdersReducer(initialOpenOrdersState, actions.userOpenOrdersUpdate({ ...newOrderEvent, state: 'done' })),
		...
		it('does not insert cancel order', () => {
			const expectedState: OpenOrdersState = { ...initialOpenOrdersState, list: [] };
			expect(
				openOrdersReducer(initialOpenOrdersState, actions.userOpenOrdersUpdate({ ...newOrderEvent, state: 'cancel' })),
			).toEqual(expectedState);
		});
		...
		it('remove done orders from the list', () => {
			...
			expect(
				openOrdersReducer({ ...initialOpenOrdersState, list: [] }, actions.userOpenOrdersUpdate(updatedOrderEvent)),
			).toEqual(expectedState);
		});
	});

	it('should handle USER_OPEN_ORDERS_RESET', () => {
		expect(openOrdersReducer(initialOpenOrdersState, actions.userOpenOrdersReset())).toEqual(initialOpenOrdersState);
	});
	...
	it('should handle OPEN_ORDERS_CANCEL_ERROR', () => {
		...
		expect(openOrdersReducer(initialState, actions.openOrdersCancelError())).toEqual(expectedState);
	});
});
